...
Expo Go・実機ビルドどちらでも動かせる状態です（ロック画面ウィジェットの検証には実機ビルドが必要）。
...
- 画面5つ：
  - 「今日」：ステータスカード表示＋TODOリスト（締め切り日・ロック画面表示トグル・
    ロック画面への反映ボタン、チェックすると「チェック済み一覧」モーダルへ移動）
  - 「カレンダー」：TimeTreeスタイルの週行グリッド、日別メモ・生理日フラグをカラーピルで表示、
    3件超は「＋N件」で折りたたみ
  - 「プレビュー」：ロック画面風モックアップでレベル1〜4を切り替えて確認（`src/screens/PreviewScreen.tsx`）
  - 「設定」：周期・画像設定・ロック画面連携の起動導線、Pro版の購入/復元。
    ネイティブ日付ピッカーやデバウンス保存など実装済み
  - 「オンボーディング」（`src/screens/OnboardingScreen.tsx`）：iOS/Android別のステップフロー、
    専用アルバムへの保存許可、「koyomi」ショートカットの追加案内・オートメーション設定手順（実機スクショ付き）、
    ロック画面TODOウィジェットの追加手順（実機スクショ付き）まで一気通貫で案内
...
- **ロック画面の壁紙連携（iOS）**：`src/services/wallpaperEngine.tsx` が中核。
  1. 画面外に常駐させたキャンバスをそのレベルの見た目で描画し、`react-native-view-shot` でキャプチャ
     （写真未設定時は同梱の静止画をそのまま使用）
...
- **ロック画面のTODOウィジェット連携（iOS）**：`@bacons/apple-targets` + WidgetKit
  （`targets/widget/TodoWidget.swift`）が中核。
  「今日」画面で反映したTODOを `src/services/widgetSync.ts` が App Group 経由の共有領域へ書き込み、
  ウィジェットの再描画を要求する。ユーザーはロック画面編集画面から「koyomi」ウィジェットを
  1度追加するだけで、以後はアプリ側の「ロック画面に反映」ボタンだけで内容が更新される
- **60日間の無料期間 → 買い切りIAP**：ロック画面の自動連携（壁紙・TODOどちらも）は初回起動から
  60日間の無料期間限定。期限は `src/logic/trial.ts` がローカルの初回起動日時から判定し、
  残り7日/1日/当日に `expo-notifications` でローカル通知（`src/services/notifications.ts`）を送る。
  買い切り版（`expo-iap`、商品ID `com.yourname.koyomi.pro`）を購入すると無期限に解除される
  （`src/services/iap.ts`、購入・復元どちらにも対応）。今日・カレンダー・TODOなど他の画面は
  期限後もそのまま使える
...
2. **EAS Build でのビルド確立**：ショートカット連携・ディープリンク・ウィジェット・
   バックグラウンド動作は Expo Go の制約を受けやすいため、実機での本格検証には
   dev client またはスタンドアロンビルドが必要
...
（ロック画面ウィジェット・IAPはネイティブモジュールのためExpo Goでは動作しません）。
ロック画面連携（ショートカット実行・ディープリンク・ウィジェット・IAP）を含めて検証する場合は、
`npx expo run:ios` で `ios/` のネイティブプロジェクトをビルドしてください
（既に `expo prebuild` 済みのため `ios/` フォルダはこのリポジトリに含まれています。
`app.json` を変更した場合は `npm run prebuild:ios` で作り直してください）。
